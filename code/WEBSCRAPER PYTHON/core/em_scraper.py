...
class EmScraper:
    def __init__(self, args):
        ...
        self.args = args
        self.config = cfg[args.school.upper()].EMAIL
        ...
        try:
            ...
            raise SystemExit
        # File Name
        self.prof_csv_file_name = f'data/{args.school}/{self.cterm[1]}/gs_{self.cterm[1]}_{args.school}_1.csv'
    ...
    def _save(self):
        ...
        self.prof_df.to_csv(index=False, path_or_buf=self.prof_csv_file_name)

    def _load(self):
        try:
            df = pd.read_csv(self.prof_csv_file_name)
            logger.info(f'Found Existing Email File')
            return df
